...
def grid_analyze_model(param_grid):
    ...
    eval.loc[len(eval.index)] = [grid.best_params_['svc__kernel'], grid.best_params_['svc__kernel'] + ' with cost {0} and gamma {1}'.format(grid.best_params_[
        'svc__C'], grid.best_params_['svc__gamma']), grid.best_score_, grid.best_estimator_.named_steps["svc"].n_support_[0], grid.best_estimator_.named_steps["svc"].n_support_[1]]
    if (grid.best_params_['svc__kernel'] == 'poly'):
        eval.loc[len(eval.index)-1,
                 'Description'] += str(grid.best_params_['svc__degree'])
    return grid.best_score_


def plot_2d(df, feature_x, feature_y):
    df["isRegular"] = df["isRegular"].astype(str)
    fig = px.scatter(df, x=feature_x, y=feature_y, color="isRegular", hover_data=[
                     'Filename'], log_x=True, log_y=True, color_discrete_sequence=["red", "blue"])
    fig.show()
...
X = data.drop(['Subject_isHealthy', 'Subject_id',
              'Filename', 'isRegular'], axis=1)
y = data['isRegular']*2-1

# Check null values
null_count = data.isnull().sum()
print('Number of features with at least one null value: {0}'.format(
...
eval = pd.DataFrame(columns=['Model', 'Description', 'Score',
                    'Number of support vectors class 0', 'Number of support vectors class 1'])
...
for cost in costs:
    ...
    eval.loc[len(eval.index)] = ['Linear', 'Linear with cost {0}'.format(
        cost), cross_val_mean, clf.n_support_[0], clf.n_support_[1]]
